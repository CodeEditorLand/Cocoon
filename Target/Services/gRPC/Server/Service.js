...
import { Context as Context6, Effect as Effect8, Layer as Layer7, Ref as Ref5, SubscriptionRef as SubscriptionRef5 } from "effect";
...
var init_RPCServer = __esm({
  async "Source/Effect/RPCServer.ts"() {
    ...
    RPCServerTag = class extends Context6.Tag("Cocoon/RPCServer")() {
      static {
        __name(this, "RPCServerTag");
      }
    };
    RPCServer = RPCServerTag;
    RPCServerLive = Layer7.effect(
      RPCServer,
      Effect8.gen(function* () {
        ...
        let startTime = 0;
        const latencies = [];
        const start = /* @__PURE__ */ __name((config) => Effect8.gen(function* () {
          ...
          try {
            ...
            yield* Effect8.promise(() => grpcServer.start());
            startTime = Date.now();
            metrics = {
              uptime: 0,
              connections: 0,
        ...
        const stop = Effect8.gen(function* () {
          const currentState = yield* stateRef.get;
          if (currentState._tag !== "Running") {
            telemetry.log("warn", "[RPCServer] Server is not running");
            return yield* Effect8.fail(new ServerNotRunningError());
          }
          yield* Ref5.set(stateRef, {
            _tag: "Stopping"
          });
          ...
          if (grpcServer) {
            yield* Effect8.promise(() => grpcServer.stop());
            grpcServer = void 0;
          }
          yield* Ref5.set(stateRef, {
            _tag: "Stopped"
        ...
        const handleRequest = /* @__PURE__ */ __name((request2) => Effect8.gen(function* () {
          const requestStartTime = Date.now();
          const currentState = yield* stateRef.get;
          if (currentState._tag !== "Running") {
            return {
        ...
        const getMetrics = Effect8.gen(function* () {
          const currentState = yield* stateRef.get;
          if (currentState._tag !== "Running") {
            return yield* Effect8.fail(new ServerNotRunningError());
          }
    ...
    makeMockRPCServer = /* @__PURE__ */ __name(() => {
      ...
      return {
        state: Effect8.succeed(mockStateRef),
        stateChanges: Effect8.succeed([mockStateRef]),
        start: /* @__PURE__ */ __name(() => Effect8.succeed(void 0), "start"),
        stop: Effect8.succeed(void 0),
        handleRequest: /* @__PURE__ */ __name((request2) => Effect8.succeed({
          requestId: request2.requestId,
          success: true,
          data: { method: request2.method, result: "mock" },
          timestamp: Date.now()
        }), "handleRequest"),
        getMetrics: Effect8.succeed({
          uptime: 0,
          connections: 0,
          requestsHandled: 0,
          errors: 0,
    ...
    RPCServerMock = Layer7.effect(
      RPCServer,
      Effect8.succeed(makeMockRPCServer())
    );
  }
});
...
import { Context as Context7, Duration, Effect as Effect9, Layer as Layer8, Schedule as Schedule2 } from "effect";
...
var init_Bootstrap = __esm({
  async "Source/Effect/Bootstrap.ts"() {
    ...
    init_Telemetry();
    ProbeTcp = /* @__PURE__ */ __name((Host, Port, TimeoutMs) => Effect9.async((Resume) => {
      let Settled = false;
    ...
    BootstrapTag = class extends Context7.Tag("Cocoon/Bootstrap")() {
      static {
    ...
    stage3_MountainConnection = withSpan(
      ...
      Effect9.gen(function* () {
        ...
        while (ProbeAttempt < MountainProbeMaxAttempts && !Listening) {
          ...
          yield* Effect9.sleep(Duration.millis(ProbeDelay));
          ProbeDelay = Math.min(
        ...
        const Connect = Effect9.gen(function* () {
          AttemptRef.value++;
    ...
    stage6_Extensions = withSpan(
      ...
      Effect9.gen(function* () {
        ...
        const ActivationAttempts = yield* Effect9.forEach(
          EligibleExtensions,
          (Ext) => extension.activate(Ext.id).pipe(
            Effect9.map(() => ({ Id: Ext.id, Ok: true })),
            Effect9.catchAll((Failure) => {
              const Message = Failure instanceof Error ? Failure.message : String(Failure);
              telemetry.log(
                "warn",
    ...
    makeBootstrap = /* @__PURE__ */ __name(() => ({
      run: /* @__PURE__ */ __name((options) => Effect9.gen(function* () {
        const telemetry = yield* TelemetryTag;
        const startTime = Date.now();
        ...
        for (const [StageName, stage] of stages) {
          ...
          const SafeStage = Effect9.suspend(() => stage).pipe(
...
`
    ...
    BootstrapLive = Layer8.effect(
      BootstrapTag,
      Effect9.succeed(makeBootstrap())
    );
    makeMockBootstrap = /* @__PURE__ */ __name(() => ({
      run: /* @__PURE__ */ __name((options) => Effect9.gen(function* () {
        yield* Effect9.sleep("1 millis");
        return {
          success: true,
          totalDuration: 1,
          stages: [
    ...
    BootstrapMock = Layer8.effect(
      BootstrapTag,
      Effect9.succeed(makeMockBootstrap())
    );
    runBootstrap = /* @__PURE__ */ __name((options) => Effect9.gen(function* () {
      const bootstrap = yield* BootstrapTag;
      return yield* bootstrap.run(options);
    }).pipe(Effect9.provide(BootstrapLive)), "runBootstrap");
  }
});
  ...
  async "Source/Service/Mapping.ts"() {
    ...
    EffectServices = {
      ...
      composeAppLayer: /* @__PURE__ */ __name(() => {
        ...
        const Layer1 = Layer9.mergeAll(
          HealthLive.pipe(Layer9.provide(Telemetry2)),
          MountainClientLive.pipe(Layer9.provide(Telemetry2)),
          ModuleInterceptorLive.pipe(Layer9.provide(Telemetry2)),
          ExtensionLive.pipe(Layer9.provide(Telemetry2)),
          RPCServerLive.pipe(Layer9.provide(Telemetry2))
        );
        const Bootstrap = BootstrapLive.pipe(
          Layer9.provide(Telemetry2),
          Layer9.provide(Layer1)
        );
        return Layer9.mergeAll(Telemetry2, Layer1, Bootstrap);
      }, "composeAppLayer"),
      /**
...
var init_Service3 = __esm({
  "Source/Interfaces/I/Extension/Host/Service.ts"() {
    "use strict";
    IExtensionHostService = Context8.Tag(
...
var init_Service4 = __esm({
  "Source/Interfaces/I/Configuration/Service.ts"() {
    ...
    IConfigurationService = Context9.Tag(
...
var init_Heuristics = __esm({
  "Source/Services/Handler/VscodeAPI/Wrap/Namespace/With/Heuristics.ts"() {
    ...
    BuildHeuristicMethod = /* @__PURE__ */ __name((NamespaceName, Property, Heuristic) => (...Arguments) => {
      ...
      try {
        return Heuristic.Sync ? Effect10.runSync(Program) : Effect10.runPromise(Program);
      } catch {
        switch (Heuristic.Kind) {
          case "trust":
...
var init_Service5 = __esm({
  "Source/Interfaces/I/Performance/Monitoring/Service.ts"() {
    "use strict";
    IPerformanceMonitoringService = Context10.Tag("IPerformanceMonitoringService");
  }
});
...
var init_Service6 = __esm({
  "Source/Interfaces/I/Security/Service.ts"() {
    "use strict";
    ISecurityService = Context11.Tag("ISecurityService");
  }
});
...
var init_Service7 = __esm({
  "Source/Interfaces/IGRPC/Server/Service.ts"() {
    "use strict";
    IGRPCServerService = Context12.GenericTag("IGRPCServerService");
  }
});
...
var init_Service8 = __esm({
  async "Source/Services/gRPC/Server/Service.ts"() {
    ...
    GRPCServerServiceLayer = Layer10.effect(
      IGRPCServerService,
      Effect11.sync(() => new GRPCServerService())
    );
    GRPCServerServiceLive = Layer10.effect(
      IGRPCServerService,
      Effect11.sync(() => new GRPCServerService())
    );
  }
