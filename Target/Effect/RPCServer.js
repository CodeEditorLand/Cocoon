...
import { Context as Context6, Duration, Effect as Effect8, Layer as Layer7, Schedule as Schedule2 } from "effect";
...
var init_Bootstrap = __esm({
  async "Source/Effect/Bootstrap.ts"() {
    ...
    init_Telemetry();
    ProbeTcp = /* @__PURE__ */ __name((Host, Port, TimeoutMs) => Effect8.async((Resume) => {
      let Settled = false;
    ...
    BootstrapTag = class extends Context6.Tag("Cocoon/Bootstrap")() {
      static {
    ...
    stage3_MountainConnection = withSpan(
      ...
      Effect8.gen(function* () {
        ...
        while (ProbeAttempt < MountainProbeMaxAttempts && !Listening) {
          ...
          yield* Effect8.sleep(Duration.millis(ProbeDelay));
          ProbeDelay = Math.min(
        ...
        const Connect = Effect8.gen(function* () {
          AttemptRef.value++;
    ...
    stage6_Extensions = withSpan(
      ...
      Effect8.gen(function* () {
        ...
        const ActivationAttempts = yield* Effect8.forEach(
          EligibleExtensions,
          (Ext) => extension.activate(Ext.id).pipe(
            Effect8.map(() => ({ Id: Ext.id, Ok: true })),
            Effect8.catchAll((Failure) => {
              const Message = Failure instanceof Error ? Failure.message : String(Failure);
              telemetry.log(
                "warn",
    ...
    makeBootstrap = /* @__PURE__ */ __name(() => ({
      run: /* @__PURE__ */ __name((options) => Effect8.gen(function* () {
        const telemetry = yield* TelemetryTag;
        const startTime = Date.now();
...
`
    ...
    BootstrapLive = Layer7.effect(
      BootstrapTag,
      Effect8.succeed(makeBootstrap())
    );
    makeMockBootstrap = /* @__PURE__ */ __name(() => ({
      run: /* @__PURE__ */ __name((options) => Effect8.gen(function* () {
        yield* Effect8.sleep("1 millis");
        return {
          success: true,
          totalDuration: 1,
          stages: [
    ...
    BootstrapMock = Layer7.effect(
      BootstrapTag,
      Effect8.succeed(makeMockBootstrap())
    );
    runBootstrap = /* @__PURE__ */ __name((options) => Effect8.gen(function* () {
      const bootstrap = yield* BootstrapTag;
      return yield* bootstrap.run(options);
    }).pipe(Effect8.provide(BootstrapLive)), "runBootstrap");
  }
});
  ...
  async "Source/Service/Mapping.ts"() {
    ...
    EffectServices = {
      ...
      composeAppLayer: /* @__PURE__ */ __name(() => {
        ...
        const Layer1 = Layer8.mergeAll(
          HealthLive.pipe(Layer8.provide(Telemetry2)),
          MountainClientLive.pipe(Layer8.provide(Telemetry2)),
          ModuleInterceptorLive.pipe(Layer8.provide(Telemetry2)),
          ExtensionLive.pipe(Layer8.provide(Telemetry2)),
          RPCServerLive.pipe(Layer8.provide(Telemetry2))
        );
        const Bootstrap = BootstrapLive.pipe(
          Layer8.provide(Telemetry2),
          Layer8.provide(Layer1)
        );
        return Layer8.mergeAll(Telemetry2, Layer1, Bootstrap);
      }, "composeAppLayer"),
      /**
...
var init_Heuristics = __esm({
  "Source/Services/Handler/VscodeAPI/Wrap/Namespace/With/Heuristics.ts"() {
    ...
    BuildHeuristicMethod = /* @__PURE__ */ __name((NamespaceName, Property, Heuristic) => (...Arguments) => {
      ...
      const Program = Effect9.gen(function* () {
        yield* Effect9.sync(() => {
          try {
      ...
      try {
        return Heuristic.Sync ? Effect9.runSync(Program) : Effect9.runPromise(Program);
      } catch {
        switch (Heuristic.Kind) {
          case "trust":
...
var init_Service7 = __esm({
  "Source/Interfaces/IGRPC/Server/Service.ts"() {
    "use strict";
    IGRPCServerService = Context11.GenericTag("IGRPCServerService");
  }
});
...
var init_Service8 = __esm({
  async "Source/Services/gRPC/Server/Service.ts"() {
    ...
    GRPCServerServiceLayer = Layer9.effect(
      IGRPCServerService,
      Effect10.sync(() => new GRPCServerService())
    );
    GRPCServerServiceLive = Layer9.effect(
      IGRPCServerService,
      Effect10.sync(() => new GRPCServerService())
    );
  }
...
import { Context as Context12, Effect as Effect11, Layer as Layer10, Ref as Ref5, SubscriptionRef as SubscriptionRef5 } from "effect";
...
var init_RPCServer = __esm({
  async "Source/Effect/RPCServer.ts"() {
    ...
    RPCServerTag = class extends Context12.Tag("Cocoon/RPCServer")() {
      static {
        __name(this, "RPCServerTag");
      }
    };
    RPCServer = RPCServerTag;
    RPCServerLive = Layer10.effect(
      RPCServer,
      Effect11.gen(function* () {
        ...
        let startTime = 0;
        const latencies = [];
        const start = /* @__PURE__ */ __name((config) => Effect11.gen(function* () {
          ...
          try {
            ...
            yield* Effect11.promise(() => grpcServer.start());
            startTime = Date.now();
            metrics = {
              uptime: 0,
              connections: 0,
        ...
        const stop = Effect11.gen(function* () {
          const currentState = yield* stateRef.get;
          if (currentState._tag !== "Running") {
            telemetry.log("warn", "[RPCServer] Server is not running");
            return yield* Effect11.fail(new ServerNotRunningError());
          }
          yield* Ref5.set(stateRef, {
            _tag: "Stopping"
          });
          ...
          if (grpcServer) {
            yield* Effect11.promise(() => grpcServer.stop());
            grpcServer = void 0;
          }
          yield* Ref5.set(stateRef, {
            _tag: "Stopped"
        ...
        const handleRequest = /* @__PURE__ */ __name((request2) => Effect11.gen(function* () {
          const requestStartTime = Date.now();
          const currentState = yield* stateRef.get;
          if (currentState._tag !== "Running") {
            return {
        ...
        const getMetrics = Effect11.gen(function* () {
          const currentState = yield* stateRef.get;
          if (currentState._tag !== "Running") {
            return yield* Effect11.fail(new ServerNotRunningError());
          }
    ...
    makeMockRPCServer = /* @__PURE__ */ __name(() => {
      ...
      return {
        state: Effect11.succeed(mockStateRef),
        stateChanges: Effect11.succeed([mockStateRef]),
        start: /* @__PURE__ */ __name(() => Effect11.succeed(void 0), "start"),
        stop: Effect11.succeed(void 0),
        handleRequest: /* @__PURE__ */ __name((request2) => Effect11.succeed({
          requestId: request2.requestId,
          success: true,
          data: { method: request2.method, result: "mock" },
          timestamp: Date.now()
        }), "handleRequest"),
        getMetrics: Effect11.succeed({
          uptime: 0,
          connections: 0,
          requestsHandled: 0,
          errors: 0,
    ...
    RPCServerMock = Layer10.effect(
      RPCServer,
      Effect11.succeed(makeMockRPCServer())
    );
  }
});
await init_RPCServer();
