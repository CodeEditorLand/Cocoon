...
var _locale = DEFAULT_LOCALE2;
var _language = DEFAULT_LANGUAGE2;
...
function InitializeDetection() {
  ...
  if (typeof nodeProcess === "object") {
    ...
    _operatingSystem = _isMacintosh ? 2 /* Macintosh */ : _isWindows ? 1 /* Windows */ : 3 /* Linux */;
    _language = navigator.language.toLowerCase().split("-")[0] || DEFAULT_LANGUAGE2;
    _locale = navigator.language || DEFAULT_LOCALE2;
...
function DetectLocaleAndLanguage(env) {
  ...
  if (rawNlsConfig) {
    try {
      ...
      _locale = nlsConfig.userLocale || DEFAULT_LOCALE2;
      _language = nlsConfig.resolvedLanguage || nlsConfig.language || DEFAULT_LANGUAGE2;
      return;
    } catch (e) {
    }
  }
  ...
  if (locale) {
    _locale = locale.split(".")[0].replace("_", "-") || DEFAULT_LOCALE2;
    _language = _locale.split("-")[0] || DEFAULT_LANGUAGE2;
    return;
  }
  _locale = DEFAULT_LOCALE2;
  _language = DEFAULT_LANGUAGE2;
}
...
function StringToPlatform(platform) {
  ...
  switch (normalized) {
    case "Web":
      return Option2.some(0 /* Web */);
    case "Mac":
      return Option2.some(1 /* Mac */);
    case "Linux":
      return Option2.some(2 /* Linux */);
    case "Windows":
      return Option2.some(3 /* Windows */);
    default:
      return Option2.none();
  }
}
...
function IsDefaultLanguage() {
  return _language === DEFAULT_LANGUAGE2;
}
...
function GetOSInfoEffect() {
  return Effect2.succeed(GetOSInfo());
}
__name(GetOSInfoEffect, "GetOSInfoEffect");
function DetectPlatformEffect() {
  return Effect2.succeed(GetPlatformNumber());
}
__name(DetectPlatformEffect, "DetectPlatformEffect");
function NormalizePathEffect(path) {
  if (!path) {
    return Effect2.fail(new Error("Path cannot be null or undefined"));
  }
  return Effect2.succeed(NormalizePath(path));
}
...
var PlatformService = class {
   ...
   */
  getLanguage() {
    return Effect4.sync(() => GetLanguage());
  }
  /**
   * Get locale
   */
  getLocale() {
    return Effect4.sync(() => GetLocale());
  }
  /**
   * Get home directory
   */
