 ...
 *     command that was registered by an extension running in Cocoon. Argument are processed
...
export class ApiCommandResult<V_InternalDTO, R_ApiType = V_InternalDTO> {
	...
	constructor(
		...
		readonly convert: (
			value: V_InternalDTO,
			uriTransformer?: IURITransformer,
			_originalApiArgument?: any[], // For context if needed during result conversion
...
const BUILTIN_COMMANDS: ReadonlyArray<ApiCommand> = [
	...
	new ApiCommand(
		...
		new ApiCommandResult<ExtHostProtocolHoverDto[], vscode.Hover[]>(
			"Array of hovers",
			(internalHovers, _uriTransformer, _origArgument, cmdConverter) => {
	...
	implements CocoonExtHostCommandsRpcShape
{
	...
	public registerApiCommand(apiCommand: ApiCommand): VscodeDisposable {
		...
		const registration = this.registerCommand(
			...
			apiCommand.id,
			async (...apiArgument: any[]) => {
				this._logService?.trace(
					`[ExtHostCommands] API command '${apiCommand.id}' invoked with ${apiArgument.length} arguments.`,
				);
				const internalArgument = apiCommand.args.map((argDef, i) => {
					const apiArgValue = apiArgument[i];
	...
	public _convertArgumentToInternalForRpc(
		args: any[],
	...
	public _reviveArgumentFromRpc(marshalledArgument: any[] | undefined): any[] {
		if (!marshalledArgument || !Array.isArray(marshalledArgument)) return [];
		return marshalledArgument.map((arg) => {
			if (
				arg &&
				(arg.$ident ||
					(typeof arg.id === "string" &&
	...
	private async _doExecuteCommand<T>(
		...
		let marshalledArgumentForRpc: any[];
		...
		if (apiCommandDef && commandId === apiCommandDef.id) {
			...
			marshalledArgumentForRpc = apiCommandDef.args.map((argDef, i) =>
			...
			marshalledArgumentForRpc = args;
		} else {
			// General command not known as API, marshal using general converter
			marshalledArgumentForRpc = this._convertArgumentToInternalForRpc(
				args,
				callDisposables,
			);
		}

		const hasBuffers = marshalledArgumentForRpc.some(
			(arg) =>
		...
		const rpcArgumentPayload = hasBuffers
			? new SerializableObjectWithBuffers(marshalledArgumentForRpc)
			: marshalledArgumentForRpc;
		...
		try {
			...
			if (originalPublicApiCommandDef) {
				...
				finalResult = this._reviveArgumentFromRpc([
					resultFromRpc,
				])[0] as T;
			}
			this._reportTelemetry(
	...
	public async $executeContributedCommand(
		commandId: string,
		marshalledArgumentFromMain: any[],
	): Promise<any> {
		this._logDebug(
			`RPC $executeContributedCommand: ID='${commandId}' from Mountain. Argument count: ${marshalledArgumentFromMain?.length || 0}`,
		);
		...
		const revivedArgumentForCallback = this._reviveArgumentFromRpc(
			marshalledArgumentFromMain,
		);

		// Further process revived args using registered ArgumentProcessors
		let processedArgumentArray = revivedArgumentForCallback;
		for (const processor of this.#argumentProcessors) {
			processedArgumentArray = processedArgumentArray.map((arg) =>
		...
		try {
			...
			try {
				...
				const marshalledResultArray =
					this._convertArgumentToInternalForRpc(
						[result],
						callDisposables,
					);
