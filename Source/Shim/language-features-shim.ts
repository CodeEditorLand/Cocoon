...
import {
	...
	// Usually Definition | Location | LocationLink | (Definition | Location | LocationLink)[]
	type Declaration as VscodeDeclaration,
	// Usually Location | LocationLink | (Location | LocationLink)[]
	...
	DefinitionLink: {
		...
		from: (
			...
			if ("targetUri" in item && "targetRange" in item) {
				...
				return {
					// Use BaseCocoonShim marshaller
					uri: base._convertApiArgToInternal(item.targetUri),

					// Range is already 0-based, DTO compatible
...
interface ProviderRegistrationEntry {
	// The actual language provider instance (e.g., HoverProvider, CompletionItemProvider)
	provider: any;

	// The DocumentSelector for this provider, null for workspace-wide providers
	selector: DocumentSelector | null;

	// A string identifying the provider type (e.g., "Hover", "Completion")
	type: string;
	...
	// Handle for onDidChangeCodeLenses event, if applicable
	codeLensEventHandle?: number;

	// Handle for onDidChangeInlayHints event
	...
	implements VscodeExtHostLanguageFeaturesShape
{
	...
	// Stores registered providers by handle
	readonly #providerStore = new Map<number, ProviderRegistrationEntry>();

	// For resolving document URIs
	...
	private async _registerProviderOnMainThread<P>(
		...
		mainThreadRegisterRpcCall: (
			...
			// Pass ExtensionIdentifier for MainThread
			extensionIdForRpc?: ExtensionIdentifier,
		) => Promise<void>,

		// Args needed for DTO conversion or local storage
		providerSpecificArgsForStorageAndDto?: any,

		// Extension registering this provider
		extensionIdForRegistration: ExtensionIdentifier = SENDER_EXTENSION_ID_FOR_REGISTRATION,
	): Promise<number> {
		// Returns the handle
		...
		const registrationEntry: ProviderRegistrationEntry = {
			...
			// Store relevant options like triggerChars, metadata
		...
		try {
			...
			await mainThreadRegisterRpcCall(
				...
				// Pass args needed for the specific DTO
				providerSpecificArgsForStorageAndDto,
	...
	public $registerDefinitionProvider(
		...
		return this._registerProviderOnMainThread(
			...
			(proxy, h, selDto, _args, extId) =>
				// Protocol uses $registerDefinitionProvider
	...
	public async $unregister(handle: number): Promise<void> {
		...
		if (!this.#mainThreadLanguageFeaturesProxy) {
			...
			// Don't throw, just log and fail to unregister on main.
			return;
		}

		try {
			// MainThread uses generic $unregister
	...
	public async $provideCompletionItems(
		...
		const context = localTypeConverters.CompletionContext.to(contextDto);
	...
	public async $resolveCompletionItem(
		...
		const itemToResolve =
			// Revive RpcSuggestData to VscodeCompletionItem
	...
	public async $provideDefinition(
		...
		try {
			...
			// Pass `this` (BaseCocoonShim) for marshalling URIs
			return localTypeConverters.DefinitionLink.fromMany(resultArr, this);
		} catch (err: any) {
			this._logError(
	...
	public async $provideCodeLenses(
		...
		try {
			...
			if (result && result.length > 0) {
				...
				// dispose on DTO is for main thread to call back
	...
	public async $resolveCodeLens(
		...
		// No resolver, return original DTO
		if (!providerMethod) return codeLensDto;
		...
		try {
			...
			// Marshal resolved VscodeCodeLens to RpcCodeLens DTO
			return this._convertApiArgToInternal(result);
		} catch (err: any) {
			this._logError(
			...
			// Marshal revived-but-unresolved
	...
	public async $provideCodeActions(
		...
		// Use local placeholder converter
		const range = localTypeConverters.Range.to(rangeDto);
		...
		try {
			...
			if (result && result.length > 0) {
				...
				// dispose on DTO for main thread
