	...
	implements IExtHostMessageServiceInterface
{
	...
	private _parseMessageArgument(
		...
		let parseStartIndex = 0; // Index into the collected allArgument array

		// Collect all potential optional arguments into a single array, filtering out undefined initial ones.
		const allPotentialArgument = [arg1, arg2, ...restItems].filter(
			(arg) => arg !== undefined,
		);
		...
		if (
			allPotentialArgument.length > parseStartIndex &&
			typeof allPotentialArgument[parseStartIndex] === "object" &&
			allPotentialArgument[parseStartIndex] !== null &&
			!(allPotentialArgument[parseStartIndex] as VscodeMessageItem).title &&
			!(allPotentialArgument[parseStartIndex] as ExtensionSourceInfo).id
		) {
			options = allPotentialArgument[parseStartIndex] as VscodeMessageOptions;
			parseStartIndex++;
		}
		...
		if (
			...
			typeof allPotentialArgument[parseStartIndex] === "object" &&
			allPotentialArgument[parseStartIndex] !== null &&
			typeof (allPotentialArgument[parseStartIndex] as ExtensionSourceInfo)
				.id === "string"
		) {
			source = allPotentialArgument[parseStartIndex] as ExtensionSourceInfo;
			parseStartIndex++;
		}

		// 3. Remaining arguments are considered action items (string or VscodeMessageItem).
		for (let i = parseStartIndex; i < allPotentialArgument.length; i++) {
			const itemCandidate = allPotentialArgument[i];
	...
	public showInformationMessage(
		...
		const { options, items, source } = this._parseMessageArgument(
			args[0],
			args[1],
			...args.slice(2),
		);
