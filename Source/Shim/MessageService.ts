	...
	implements IExtHostMessageServiceInterface
{
	...
	private _parseMessageArgument(
		...
		if (
			...
			!(allPotentialArgument[parseStartIndex] as VscodeMessageItem)
				.title &&
			!(allPotentialArgument[parseStartIndex] as ExtensionSourceInfo).id
		) {
			options = allPotentialArgument[
				parseStartIndex
			] as VscodeMessageOptions;
			parseStartIndex++;
		}
		...
		if (
			...
			allPotentialArgument[parseStartIndex] !== null &&
			typeof (
				allPotentialArgument[parseStartIndex] as ExtensionSourceInfo
			).id === "string"
		) {
			source = allPotentialArgument[
				parseStartIndex
