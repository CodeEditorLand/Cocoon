...
import {
	// Renamed to avoid confusion if used locally
	Emitter as VscodeEmitter,
	Event as VscodeEvent,
	type IDisposable,
} from "vs/base/common/event";
import {
	// For checking marshalled types
	MarshalledId,
	// VS Code's standard revival function
	revive as vscodeRevive,
	// Not directly used in this file's API
...
export interface ProxyIdentifier<T> {
	...
	// Though nid is not always used by all RPCProtocol versions.
	readonly nid: number;
}
...
interface IStructuredErrorPayload {
	...
	// Node.js 'code' (e.g., 'ENOENT') or custom numeric code
	code?: string | number;

	// POSIX errno number
	errno?: number;

	// System call related to the error (e.g., 'stat')
	syscall?: string;
...
export function refineErrorForShim(
	// Accept any to handle non-Error throws
	originalError: any,
...
export class BaseCocoonShim implements IDisposable {
	...
	// For managing disposables within the shim instance
	...
	constructor(
		...
		if (!this.#rpcProtocolAdapter && this._requiresRpc()) {
			...
			// Use internal logger which falls back to console
			this._logError(
	...
	protected _getProxy<T>(identifier: ProxyIdentifier<T>): T | null {
		...
		if (!this.#rpcProtocolAdapter) {
			this._logError(
				`Cannot get RPC proxy for ${serviceSidForLog}: RPCService Adapter (IRpcProtocolServiceAdapter) is unavailable.`,
			);
	...
	protected async _ipcRequestResponse(
		...
		try {
			...
			this._logError(
				`IPC Req Error for '${mountainMethod}': ${refined.message}`,
				refined.stack,
			);
	...
	protected _convertApiArgToInternal(arg: any): any {
		...
		// VSBuffer is handled by RPCProtocol
		...
		try {
			...
			if (arg instanceof VscodeApiUri) {
				return {
					// Or MarshalledId.Uri if full components always needed
					...
					// Optional: useful for debugging on other side
			...
			if (arg instanceof VscodeApiSelection) {
				...
				return {
					// Ensure this ID exists or use a custom one
					$mid: MarshalledId.Selection,
					// 1-based for protocol
		...
		if (typeof (arg as any).toJSON === "function" && !Array.isArray(arg)) {
			try {
				...
				this._logWarn(
					"Call to toJSON() failed on argument, proceeding with recursive conversion:",
					arg,
					e,
				);
	...
	protected _createEventEmitter(): EventEmitter {
		...
		// Consider if default of 10 is too low.
	...
	protected _createEventFromEmitter<T>(
		...
		const event: VscodeEvent<T> = (listener, thisArgs?, disposables?) => {
			if (typeof listener !== "function") {
				this._logError(
					`_createEventFromEmitter: listener is not a function for event '${eventName}'`,
					listener,
				);
	...
	protected _createNopEventEmitter(): VscodeEvent<any> {
		if (VscodeEvent && typeof VscodeEvent.None === "function") {
			// Use VS Code's official NOP event if available
			return VscodeEvent.None;
		}

		this._logWarnOnce(
			"VscodeEvent.None not available or failed, using NOP event stub.",
		);
