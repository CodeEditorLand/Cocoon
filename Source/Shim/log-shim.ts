...
// For path.sep in deriving logger name
...
import {
	// Helper to parse string log levels
	parseLogLevel,
...
export class ShimLogService implements VscodeILogService {
	...
	constructor(
		...
		// Use trace to avoid noise if initial level is higher
		this.trace(
	...
	public critical(message: string | Error, ...args: any[]): void {
		...
		// Check against Critical if it's distinct from Error
	...
	public setLevel(level: VscodeLogLevel | string): void {
		...
		if (
			newLevel === undefined ||
			!Object.values(VscodeLogLevel).includes(newLevel)
		) {
			const namePrefix = this.loggerInstanceName
				? `[${this.loggerInstanceName}] `
				: "";
			...
			console.warn(
				`${namePrefix}[Cocoon Log Shim] Invalid log level string or value: ${level}. Level not changed.`,
			);
	...
	public createLogger(
		...
		// Inherit parent's level by default
...
export class ShimLoggerService implements VscodeILoggerService {
	...
	// Payload: [resource URI | undefined for default, newLevel]
	...
	public createLogger(
		...
		options?: {
			...
			logLevel?: VscodeLogLevel /* ; File?: any FileLoggerOptions */;
		},
	): VscodeILogger {
		...
		if (
			!this.loggersByResource.has(resourceKey) ||
	...
	// Can return undefined if no specific level set
	public getLogLevel(resource?: URI): VscodeLogLevel | undefined {
		if (resource) {
			...
			// No specific logger, so no specific level known by LoggerService itself
			return undefined;
		}
	...
	public setLogLevel(
		...
		if (resourceOrLevel instanceof URI) {
			...
			// Should not happen with TS overload, but guard
			if (levelValue === undefined) {
				console.warn(
					"[Cocoon LoggerService Shim] setLogLevel called with URI but no level value.",
				);
		...
		if (
			...
			console.warn(
				`[Cocoon LoggerService Shim] Invalid log level value: ${newLevel}. Level not changed.`,
			);
