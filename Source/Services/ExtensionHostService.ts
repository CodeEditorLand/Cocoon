...
export class ExtensionHostService implements IExtensionHostService {
     ...
     */
    async activateExtension(extensionId: string, reason: ExtensionActivationReason): Promise<ActivatedExtension> {
        if (!this._readyToRunExtensions) {
            throw new Error('Extension host not ready to run extensions');
        }
        
        const extension = this._extensionRegistry.getExtensionDescription(extensionId);
        if (!extension) {
            throw new Error(`Extension ${extensionId} not found`);
        }
        
        console.log(`[ExtensionHostService] Activating extension: ${extensionId}, reason: ${reason.activationEvent}`);
        ...
        try {
            ...
            console.log(`[ExtensionHostService] Extension ${extensionId} activated successfully`);
     ...
     */
    private async _callActivate(extensionModule: any, extension: IExtensionDescription): Promise<any> {
        if (typeof extensionModule.activate !== 'function') {
            throw new Error(`Extension ${extension.identifier} has no activate function`);
        }
    ...
    async deactivateExtension(extensionId: string): Promise<void> {
        ...
        console.log(`[ExtensionHostService] Deactivating extension: ${extensionId}`);
        
        try {
            // Notify Mountain about deactivation
            await this.ipcService.send('extension-deactivation-start', { extensionId });
            ...
            await this.ipcService.send('extension-deactivation-complete', { extensionId });
            
            console.log(`[ExtensionHostService] Extension ${extensionId} deactivated`);
            
        } catch (error) {
            console.error(`[ExtensionHostService] Failed to deactivate extension ${extensionId}:`, error);
            throw error;
        }
    }
    ...
    async terminate(reason: string, code: number = 0): Promise<void> {
        ...
        console.log(`[ExtensionHostService] Terminating extension host: ${reason}`);
        
        try {
            // Deactivate all extensions
            const deactivationPromises = Array.from(this._activatedExtensions.keys())
