...
export class ExtensionChannel implements IServerChannel<any> {
    ...
    private async handleActivateExtension(arg: any): Promise<any> {
        ...
        try {
            const activatedExtension = await this.extensionHostService.activateExtension(
                extensionId,
                { startup: true, activationEvent, extensionId }
            );
            ...
            console.error(`[ExtensionChannel] Failed to activate extension ${extensionId}:`, error);
            throw error;
        }
    }
    ...
    private async handleDeactivateExtension(arg: any): Promise<any> {
        ...
        console.log(`[ExtensionChannel] Deactivating extension: ${extensionId}`);
        ...
        try {
            ...
            console.error(`[ExtensionChannel] Failed to deactivate extension ${extensionId}:`, error);
            throw error;
        }
    }
