    ...
    return {
        ...
        workspace: {
            ...
            getConfiguration: (section?: string) => {
                return {
                    ...
                    has: (key: string) => configService.hasKey(section ? `${section}.${key}` : key, 0),
                    inspect: (key: string) => configService.inspect(section ? `${section}.${key}` : key, 0)
                };
            },
        ...
        scm: {
            createSourceControl: (id: string, label: string) => ({
                 createResourceGroup: (id: string, label: string) => ({ resourceStates: [] }),
                 dispose: () => {}
            })
        },
        authentication: {
