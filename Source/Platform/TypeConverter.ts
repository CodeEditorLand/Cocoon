...
export function ConvertPlatformNumberToDTO(platformNumber: number): number {
    ...
    if (platformNumber < 0 || platformNumber > 3) {
        console.warn(`[TypeConverter] Invalid platform number: ${platformNumber}, using default`);
...
export function ConvertOperatingSystemToNumber(os: number): number {
    if (os < 1 || os > 3) {
        console.warn(`[TypeConverter] Invalid operating system: ${os}, using default`);
...
export function ConvertOSInfoToDTO(osInfo: any): MountainPlatformInfoDTO {
    ...
    return {
        platform_number: ConvertPlatformNumberToDTO(osInfo.platformNumber || osInfo.platform),
        platform_name: String(osInfo.platform || osInfo.platformName),
        operating_system: ConvertOperatingSystemToNumber(osInfo.operatingSystem || osInfo.OS),
        architecture: ConvertArchitectureToString(osInfo.architecture || 'unknown'),
 ...
 */
export function ConvertEnvironmentInfoToDTO(envInfo: any): MountainEnvironmentInfoDTO {
    return {
        ...
        variable_count: Number(envInfo.variables ? Object.keys(envInfo.variables).length : 0),
 ...
 */
export function ConvertDTOToEnvironmentInfo(dto: MountainEnvironmentInfoDTO): any {
    return {
        language: dto.language,
        locale: dto.locale,
...
export function ConvertProcessInfoToDTO(procInfo: any): MountainProcessInfoDTO {
    ...
    return {
        ...
        end_time: procInfo.status === 'stopped' ? (procInfo.endTime ?? now) : undefined,
 ...
 */
export function ConvertProcessSpawnOptionsToDTO(options: any): MountainProcessSpawnOptionsDTO {
    return {
 ...
 */
export function ConvertDTOToProcessSpawnOptions(dto: MountainProcessSpawnOptionsDTO): any {
    return {
        cwd: dto.cwd,
 ...
 */
export function DeserializeDTO<T>(json: string, validator?: (obj: any) => boolean): T | null {
    try {
 ...
 */
export function ValidateEnvironmentVariableDTO(dto: MountainEnvironmentVariableDTO): boolean {
    return (
        typeof dto === 'object' &&
 ...
 */
export function DeserializeDTOEffect<T>(json: string, validator?: (obj: any) => boolean): Option.Option<T> {
