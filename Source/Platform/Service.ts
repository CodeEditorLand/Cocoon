...
import { Effect, Layer, Context, Option, Array, HashMap } from "effect";
...
export interface IPlatformService {
    ...
    executeCommand(
        ...
        args: string[],
        options?: ProcessModule.ProcessSpawnOptions
    ): Effect.Effect<{ stdout: string; stderr: string; exitCode: number | null }, Error>;
...
export class PlatformService implements IPlatformService {
    ...
    initialize(): Effect.Effect<void, Error> {
        return Effect.sync(() => {
            ...
            this.cache.set('platform', { value: platform, timestamp: Date.now() });
            this.cache.set('osInfo', { value: osInfo, timestamp: Date.now() });
            this.cache.set('envInfo', { value: envInfo, timestamp: Date.now() });
    ...
    detectPlatform(): Effect.Effect<OSModule.PlatformNumber, Error> {
        return Effect.sync(() => {
            return this.getCached('platform', () => OSModule.GetPlatformNumber());
        });
    }
     ...
     */
    getEnvironmentVariable(name: string): Effect.Effect<Option.Option<string>> {
        return Effect.sync(() => EnvironmentModule.GetEnvironmentVariable(name));
    }
    ...
    getEnvironmentInfo(): Effect.Effect<EnvironmentModule.EnvironmentInfo, Error> {
        return Effect.sync(() => {
            return this.getCached('envInfo', () => EnvironmentModule.GetEnvironmentInfo());
        });
    }
    ...
    executeCommand(
        ...
        return Effect.tryPromise({
            try: () => ProcessModule.ExecuteCommand(command, args, options || {}),
     ...
     */
    convertOSInfoToDTO(osInfo: OSModule.OSInfo): Effect.Effect<TypeConverterModule.MountainPlatformInfoDTO> {
        return Effect.sync(() => TypeConverterModule.ConvertOSInfoToDTO(osInfo));
    }
     ...
     */
    convertEnvironmentInfoToDTO(envInfo: EnvironmentModule.EnvironmentInfo): Effect.Effect<TypeConverterModule.MountainEnvironmentInfoDTO> {
        return Effect.sync(() => TypeConverterModule.ConvertEnvironmentInfoToDTO(envInfo));
    }
     ...
     */
    convertProcessInfoToDTO(procInfo: ProcessModule.ProcessInfo): Effect.Effect<TypeConverterModule.MountainProcessInfoDTO> {
        return Effect.sync(() => TypeConverterModule.ConvertProcessInfoToDTO(procInfo));
    }
 ...
 */
export const PlatformServiceTag = Context.GenericTag<IPlatformService>("PlatformService");

/**
 * Platform Service Layer
 */
export const PlatformServiceLayer = Layer.sync(PlatformServiceTag, () => new PlatformService());
 ...
 */
export function DetectPlatform(): Effect.Effect<OSModule.PlatformNumber, never, IPlatformService> {
 ...
 */
export function GetOSInfo(): Effect.Effect<OSModule.OSInfo, never, IPlatformService> {
    return Effect.flatMap(
        Effect.service(PlatformServiceTag),
 ...
 */
export function NormalizePath(path: string): Effect.Effect<string, never, IPlatformService> {
    return Effect.flatMap(
        Effect.service(PlatformServiceTag),
 ...
 */
export function GetEnvironmentVariable(name: string): Effect.Effect<Option.Option<string>, never, IPlatformService> {
    return Effect.flatMap(
        Effect.service(PlatformServiceTag),
 ...
 */
export function SetEnvironmentVariable(name: string, value: string): Effect.Effect<void, never, IPlatformService> {
    return Effect.flatMap(
        Effect.service(PlatformServiceTag),
...
export function ExecuteCommand(
    ...
    args?: string[],
    options?: ProcessModule.ProcessSpawnOptions
): Effect.Effect<{ stdout: string; stderr: string; exitCode: number | null }, never, IPlatformService> {
    return Effect.flatMap(
        Effect.service(PlatformServiceTag),
