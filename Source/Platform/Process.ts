...
export async function SpawnProcess(
    ...
    try {
        ...
        childProc.on('exit', (code: number | null, signal: NodeJS.Signals | null) => {
            ...
            console.log(`[Process] Process ${childProc.pid} exited: code=${code}, signal=${signal}`);
        });
        ...
        console.log(`[Process] Spawned process: pid=${childProc.pid}, command=${command}`);
        return processInfo;
    } catch (error) {
...
export async function ExecuteCommand(
    ...
    return new Promise((resolve) => {
        ...
        childProcess.execFile(command, args, execOptions, (error: any, stdout: string, stderr: string) => {
            if (error) {
                resolve({
                    stdout: stdout || '',
...
export async function ForkProcess(
    ...
    try {
        ...
        console.log(`[Process] Forked process: pid=${childProc.pid}, module=${modulePath}`);
        return processInfo;
    } catch (error) {
 ...
 */
export function SendSignal(pid: number, signal: NodeJS.Signals | string): boolean {
    try {
        process.kill(pid, signal);
        return true;
    } catch (error) {
        console.error(`[Process] Failed to send signal ${signal} to pid ${pid}:`, error);
        return false;
    }
}
 ...
 */
export function TerminateProcess(pid: number, timeout: number = DEFAULT_KILL_TIMEOUT): boolean {
...
export function GetRunningProcesses(): ProcessInfo[] {
    return Array.from(ProcessRegistry.values()).filter(p => p.status === 'running');
}
...
export function GetStoppedProcesses(): ProcessInfo[] {
    return Array.from(ProcessRegistry.values()).filter(p => p.status === 'stopped' || p.status === 'error');
}
 ...
 */
function GetMergedEnvironment(additionalEnv?: Record<string, string>): Record<string, string> {
 ...
 */
export function SendSignalEffect(pid: number, signal: NodeJS.Signals): Effect.Effect<void, Error> {
    return Effect.try(() => {
        if (!SendSignal(pid, signal)) {
            throw new Error(`Failed to send signal ${signal} to process ${pid}`);
        }
    });
}
 ...
 */
export function GetProcessEffect(pid: number): Effect.Effect<Option.Option<ProcessInfo>> {
