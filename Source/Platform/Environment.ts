 ...
 */
export function GetEnvironmentVariableOr(name: string, defaultValue: string): string {
...
export function GetPlatformHome(): string {
    ...
    switch (platform) {
        case 'mac':
            return `${home}/Library`;
        case 'windows':
            return GetEnvironmentVariableOr('APPDATA', `${home}/AppData/Roaming`);
        case 'linux':
        default:
            return home;
    }
...
function GetPlatformType(): 'windows' | 'mac' | 'linux' {
    ...
    const platform = currentProcess && 'platform' in currentProcess ? (currentProcess as any).platform as string : '';
 ...
 */
export function GetEnvironmentVariableEffect(name: string): Effect.Effect<Option.Option<string>> {
 ...
 */
export function GetEnvironmentVariableOrEffect(name: string, defaultValue: string): Effect.Effect<string> {
 ...
 */
export function SetEnvironmentVariableEffect(name: string, value: string): Effect.Effect<void, Error> {
    if (!name) {
        return Effect.fail(new Error('Environment variable name cannot be empty'));
    }
    return Effect.sync(() => {
