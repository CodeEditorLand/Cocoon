...
function InitializeDetection(): void {
    ...
    if (typeof nodeProcess === 'object') {
        ...
        _operatingSystem = _isMacintosh ? OperatingSystem.Macintosh :
                          _isWindows ? OperatingSystem.Windows :
...
export function GetPlatformName(): PlatformName {
    switch (_platformNumber) {
        case PlatformNumber.Web: return 'Web';
        case PlatformNumber.Mac: return 'Mac';
        case PlatformNumber.Linux: return 'Linux';
        case PlatformNumber.Windows: return 'Windows';
    }
}
 ...
 */
export function NormalizePathToWindows(path: string | null | undefined): string {
    if (!path) {
        return '';
    }
...
export function JoinPath(...segments: (string | null | undefined)[]): string {
    const validSegments = segments.filter(s => s != null && s !== '') as string[];
 ...
 */
export function StringToPlatform(platform: string): Option.Option<PlatformNumber> {
    const normalized = String(platform);
    switch (normalized) {
        case 'Web': return Option.some(PlatformNumber.Web);
        case 'Mac': return Option.some(PlatformNumber.Mac);
        case 'Linux': return Option.some(PlatformNumber.Linux);
        case 'Windows': return Option.some(PlatformNumber.Windows);
 ...
 */
export function NormalizePathEffect(path: string): Effect.Effect<string, Error> {
    if (!path) {
