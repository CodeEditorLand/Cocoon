	...
	constructor(
		...
		this._commandsImpl.registerCommand(
			...
			async (ident: string, ...restArgumentFromMainThread: any[]) => {
				...
				let executionArgument = command.arguments || [];
				if (restArgumentFromMainThread.length > 0) {
					this._logService?.warn(
						`[CC] Delegating command for $ident '${ident}' received unexpected additional arguments from MainThread:`,
						restArgumentFromMainThread,
		...
		if (typeof commandDto.$ident === "string") {
			...
			const originalCmdId =
				commandDto.id === this.delegatingCommandId
					...
					: commandDto.id;
			const revivedArgument = commandDto.arguments
				? (this._commandsImpl as any)._reviveArgumentFromRpc(
