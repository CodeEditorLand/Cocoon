...
/*---------------------------------------------------------------------------------------------
 * Cocoon API Type Converters - Completion
 * --------------------------------------------------------------------------------------------
 * Contains converters related to code completion (IntelliSense) features, handling the
 * transformation between `vscode` API types and internal DTOs.
 *--------------------------------------------------------------------------------------------*/
...
export namespace Suggest {
		...
		if (item.command) {
			...
			if (commandDto) {
				resultDto[extHostProtocol.ISuggestDataDtoField.commandId] =
					commandDto.id;
				resultDto[
					extHostProtocol.ISuggestDataDtoField.commandArgument
	...
	export function fromList(
		...
		for (let i = 0; i < suggestionsFromApi.length; i++) {
			...
			if (!item) continue;
			const dto = from(item, commandsConverter, disposables);
			(dto as any).x = [providerId, i];
