...
/*---------------------------------------------------------------------------------------------
 * Cocoon API Type Converters - Language Features
 * --------------------------------------------------------------------------------------------
...
export namespace SemanticTokens {
	...
	export function toApi(
		...
		return new extHostTypes.SemanticTokens(
			new Uint32Array(dto.data as number[]),
			dto.resultId,
		);
	}
}
...
export namespace CallHierarchyItem {
	...
	export function toApi(
		...
		(result as any)._sessionId = dto._sessionId;
		(result as any)._itemId = dto._itemId;
		if (dto.tags) {
	...
	export function fromApi(
		...
		return {
			...
			location: part.location
				? LocationConverter.from(part.location, uriTransformer)
				: undefined,
...
export namespace InlayHint {
	...
	export function fromApiList(
		...
		return { hints: resultHints, cacheId: cacheId };
	}
}
...
export namespace WorkspaceSymbol {
	export function fromApi(
		...
		return {
			...
			location: LocationConverter.from(info.location, uriTransformer),
		};
	}
	export function toApi(
