...
class PerformanceBenchmark {
	...
	async benchmarkAPIFactoryService() {
			...
			constructionRequests.push(
				apiFactory.createVSCodeAPI({
					...
					extensionDescription: {
						extensionLocation: `/extensions/test-${i}`,
					},
					securityContext: {},
				}),
			);
		...
		for (let i = 0; i < 50; i++) {
			await apiFactory.createVSCodeAPI({
				...
				apiVersion: "1.85.0",
				extensionDescription: {
					extensionLocation: "/extensions/cached",
				},
				securityContext: {},
			});
		}
	...
	async benchmarkModuleInterceptorService() {
		...
		for (let i = 0; i < 20; i++) {
			securityTests.push(() =>
				interceptor.analyzeModuleSecurity(`test-module-${i}`),
			);
		}
	...
	printResults() {
		...
		if (failed === 0) {
			console.log(
				"🎉 All benchmarks passed! Cocoon is ready for production.",
			);
		} else {
