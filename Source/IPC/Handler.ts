...
import { Result, type Ok, type Err } from '../Utility/Result';
...
export class IPCHandler {
    ...
    public async RegisterHandler<TInput, TOutput>(
        ...
        try {
            ...
            this.logger.error(`Failed to register handler for method: ${method}`, err);
    ...
    public async HandleRequest<TInput, TOutput>(
        ...
        try {
            ...
            const err = error instanceof Error ? error : new Error(String(error));
            this.logger.error(`Request failed for method: ${request.method}`, err, { requestId });
    ...
    public UnregisterHandler(method: string): Result<boolean, Error> {
        try {
            ...
            this.logger.info(
                `Handler unregistered for method: ${method}`,
                { existed }
            );
            ...
            const err = error instanceof Error ? error : new Error(String(error));
            this.logger.error(`Failed to unregister handler for method: ${method}`, err);
            return Result.Err(err);
        }
    }
    ...
    public Dispose(): void {
        ...
        for (const tokenSource of this.pendingRequests.values()) {
            try {
                ...
                this.logger.warn('Failed to cancel pending request during disposal', error);
            }
        }
     ...
     */
    private UpdateStats(method: string, startTime: number, success: boolean): void {
