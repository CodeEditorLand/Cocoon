...
export class CommandService extends Effect.Service<CommandService>()(
	"Service/Command",
	{
		...
		effect: Effect.gen(function* () {
			...
			// The `Telemetry` and `Window` services were listed as dependencies for the
			...
			const RegisterCommand = (
				...
				return IPCService.SendNotification("$registerCommand", [Id]);
			};
			...
			const ExecuteCommand = <T>(
				...
				return IPCService.SendRequest<T>("$executeCommand", [
					Id,
					...Arguments,
				]);
			};
			...
			const GetCommands = (
				...
				return IPCService.SendRequest<string[]>("$getCommands", [
					FilterInternal,
				]);
			};

			const ServiceInstance: IExtHostCommands = {
